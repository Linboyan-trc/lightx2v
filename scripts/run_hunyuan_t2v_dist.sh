...
# 1.5 启用多进程，指定项目入口
# 1.5.1 启用了多进程，指定进程数量
# 1.5.2 指定项目入口__main__.py
# 1.5.3 指定模型、模型路径、prompt、task
# 1.5.4 指定配置文件，含有: 推理步数、视频长、宽、高、attention_type、seed、mm_config，用于加速计算
# 1.5.5 指定配置文件，含有: parallel_attn_type，指定并行注意力算法为ulysses
# 1.5.6 指定保存路径
...
# 1.6.4 指定配置文件，含有: 推理步数、视频长、宽、高、attention_type、seed、mm_config，用于加速计算
# 1.6.5 指定配置文件，含有: parallel_attn_type，指定并行注意力算法为ring
# 1.6.6 指定保存路径
# 1.6.7 两个torchrun顺序执行，第一个torchrun运行完，才会运行第二个torchrun
torchrun --nproc_per_node=4 ${lightx2v_path}/lightx2v/__main__.py \
--model_cls hunyuan \
--model_path $model_path \
--prompt "A cat walks on the grass, realistic style." \
